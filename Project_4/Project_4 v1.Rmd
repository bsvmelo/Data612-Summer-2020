...
We can see that the UBCF model is the more accurate of the two Next, we look at the RMSE, MSE and MAE. SVD is the better model, producing the lowest errors.  
...
A good performance index is the area under the curve (AUC), i.e. the area under the ROC curves. Curve above seems to favor the UBCF model but let's calculate the AUC proper. AUC is calculated as the area formed by the TPR and FPR coordinates. To calculate the area, we will do the following procedure:
...
# Increase in serendipity (Deliverable 2)
...
Our methodology is to include ramdonly sampled movies that contain some level of seredenpity into the training dataset, and then measure this inclusion impact. To avoid increasing the size of the training dataset, we will reduce its size accordingly before inclusion of the serendipity ratings. 

We will measure the inclusion impact varying the number of seredenpity movies into the training set. This to be done through a for loop from 10 to 100%.
...
# error tables RMSE
k_Method <- c("UBCF-Cosine","SVD")
k_table_s <- data.frame(rbind(ubcf_er_s, svd_er_s))
rownames(k_table_s) <- k_Method
#k_table_s <- k_table_s[order(k_table_s$RMSE ),]
kable(k_table_s) %>% kable_styling()

# ROC Curve plot
c<-lapply(vec_size,function(n){plot(list_results_s[[n]], annotate = 1, legend = "topleft")})
title("ROC Curve")
# Precision-Recall plot
d<-lapply(vec_size,function(n){plot(list_results_s[[n]], "prec/rec", annotate = 1, legend = "bottomright")})
title("Precision-Recall")
...
#AUC calculation using Trapezoid Rule Numerical Integration
auc_ubcf_s<-lapply(vec_size,function(n){trapz(norm_x_s[[n]],norm_y_s[[n]])})
#SVD
...
#AUC calculation using Trapezoid Rule Numerical Integration
auc_svd_s<-lapply(vec_size,function(n){trapz(norm_z_s[[n]],norm_w_s[[n]])})

#k_Method <-lapply(vec_size,function(n){c("UBCF-Cosine", "SVD")})
auc_table<- data.frame(rbind(auc_ubcf_s, auc_svd_s))
rownames(auc_table) <- k_Method
#k_table_p <-lapply(vec_size,function(n){rownames(k_table_p[[n]]) <- k_Method[[n]][1:2]})
#k_table_s <-lapply(vec_size,function(n){data.frame(rbind(cbind(AUC=c(auc_ubcf_s[[n]],auc_svd_s[[n]]))))})
kable(auc_table) %>% kable_styling()
