...
There are `r n_items_0` items from a total of `r n_items` that have no reviews, so let's remove them and add items that have been reviewed at least 5 times but less than 25 times.
...
Given size, let's use the top 2% to work with ratings preediction.
```{r}
...
nrow(getData(ev_boot,"train"))/nrow(redux)

ev_test_boot<-as(ev_boot@unknownData,"matrix")
nrow(getData(ev_boot,"unknown"))/nrow(redux)
...
eva_accu<-calcPredictionAccuracy(x=ev_split_pred,data=getData(ev_split,"unknown"),byUser=FALSE)
head(eva_accu)
...
eva_accu_boot<-calcPredictionAccuracy(x=ev_boot_pred,data=getData(ev_boot,"unknown"),byUser=FALSE)
head(eva_accu_boot)
...
eva_accu_fold<-calcPredictionAccuracy(x=ev_fold_pred,data=getData(ev_fold,"unknown"),byUser=FALSE)
head(eva_accu_fold)
