...
- items that have been reviewed at least 100 times but less than 300 times.
...
Let's check the number of items that have been rated.

```{r}
n_items<-colCounts(ratings)
qplot(n_items[n_items<25]) + stat_bin(binwidth = 0.1) +ggtitle("Distribution of ratings")
...
```{r}
# Items subsetting
ratings<-ratings[,colCounts(ratings)>0] 
ratings
...
Plot below shows the rating distribution.

```{r}
vector_ratings<-as.vector(getRatings(ratings))
(table_vec_ratings<-table(vector_ratings))
```


# Heatmap visualization

Visualization of the top 5 percent of users and items in the next subset matrix.

```{r}
min_inst<-quantile(rowCounts(ratings),0.98)
min_users<-quantile(colCounts(ratings),0.98)
...
```{r}
min_inst<-quantile(rowCounts(ratings_norm),0.97)
min_users<-quantile(colCounts(ratings_norm),0.97)
...
```{r}
ratings_good<-binarize(ratings, minRating=4)
```

Following plot shows the heatmap of the top users and movies, after binarization

```{r}
min_itm_bin<-quantile(rowCounts(ratings),0.97)
min_users_bin<-quantile(colCounts(ratings),0.97)
...
#  Similarities between top 3% users
...
Normalized dataset looks less similar than the non-normalized dataset.

Let's visualize the binarized dataset, using the *jaccard* method.

```{r}
# Binarized dataset
sim_users_bin<-similarity(ratings_good[rowCounts(ratings)>min_inst,colCounts(ratings)>min_users], method = "jaccard", which="users")
...
#  Similarities between top 10% items
...
This matrix looks less similar than the previous one.

```{r}
# binarized dataset
sim_item_bin<-similarity(ratings_good[rowCounts(ratings)>min_inst,colCounts(ratings)>min_users], method = "jaccard", which="items")
image(as.matrix(sim_item_bin, main="Item Similarity - Binarized DataSet"))

```

As before, the binarized dataset is very dissimilar.
