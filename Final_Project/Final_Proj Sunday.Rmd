...
rating_raw<-rbind(ratings_raw1,ratings_raw1)
rating_raw$rating[(rating_raw$rating) == -1 ] <- 0
...
We build a Content based recommendation system based on genre similarity and based on popularity. Approach is to take column "genre" in the dataset, which displays multiple tagged genres and build recommendations around it. 
...
Popularity will be measured by the number of times an anime has been viewed, based on the ratings dataset. Ratings dataset is a typical item-user matrix, and with the use of the function colCounts from the recommenderlab package, a frequency term will be calculated.

First, we will conform the rating dataset to the dataset used above. Next step is to create a viewer frequency or popularity dataset. We will then transpose the binary matrix of animes by genres created above.

In order to incorporate the popularity dataset into the binary matrix, our approach is to divide the number of views by the number of genres an anime has. To avoid penalizing animes that have many genre classifications, that result will be weighted by the square root of total numbers of genres.

Final goal is to randomly select a genre, and display a top N recommendation list based on that index.
...
rating_raw<-rating_raw[rating_raw$user_id %in% anime_genre_ids,]
...
#insert number of views in each column, weigthed by square root of total numbers of genres
...
# Top N Recommendation by popularity

Below we implement the recommendation list based on the popularity matrix calculated above. One genre is randomly sampled and the N most popular animes related to that genre is displayed.

```{r}
# set number of recommendations
n_recommended <- 10
# randomly select a genre and get corresponding name 
...
# create table
header <- sprintf("Most viewed animes in genre: %s", s_genre)
# display the list of similar animes
...
# Least popular but with high ratings
To add a surprise element to the list generated above, we will populate the top N recommendation by popularity using a list of animes that are the least popular but have high ratings.

As highlight in the chart, there are some animes that might not be picked by our popularity system because those groups have few members. In fact, from that least popular group, only 2 animes are part of the popularity matrix calculated above. This can be seen below.
```{r}
...
tmp
```
These two animes will be dropped from this dataset and we will proceed with buiding a surprise popularity matrix using the same procedure as previously described.
...
Similarity matrix will be interactively built based on the anime choosen in the Top N Recommendation by genre similarity module above.
...
Below we implement the recommendation list based on the similarity matrix calculated above. Based on the anime that was selected above, top N most similar animes to the one sampled is displayed.
...
# create table
header <- sprintf(" Surprise! These animes are not well known but similar to: %s", a_name)
# display the list of similar animes
...
#ratings matrix
rating_raw<-rating_raw[rating_raw$user_id %in% anime_genre_ids,]
rating_mtx <- as(rating_raw, "realRatingMatrix")
# split datasets
